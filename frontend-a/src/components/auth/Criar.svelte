<script lang="ts">
  ...
  const autoPreecher = () => {
    nome = "ian";
    email = "ianleonardoaraujo@gmail.com";
    telefone = "22974018011";
    senha = "123456";
    repitaSenha = "123456";
  };
  ...
  async function pegarFormulario(
    ...
    await fetch(url.criarConta, {
      ...
      .then(async (res) => {
        ...
        if (res.status !== 201) {
          toast.error(body.mensagem, { position: "top-right" });
          return;
        }
        if (res.status === 201) {
          toast.success("Conta criada com sucesso!", { position: "top-right" });
          toast.success("Aguarde até que o administrador libere seu acesso!", {
            position: "top-right",
          });
        }
      })
      .catch((err) => {
        toast.error("Erro ao criar conta!", { position: "top-right" });
        return;
      })
      .finally(() => {
  ...
  >
    ...
    >
      <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
        ...
        <form class="space-y-4 md:space-y-6" on:submit={pegarFormulario}>
          <div>
            <label
              for="nome"
            ...
            <div class="ml-3 text-sm">
              <label
                ...
                >Eu aceito os <a
                  ...
                  href="#email">Termos e condições</a
                ></label
              >
            </div>
          </div>
